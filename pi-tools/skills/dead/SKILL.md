...
## Confidence-driven flow (resolve like bug/formatter)
...
3. **High confidence (zero references, no dynamic/config/entry/public reference) -> REMOVE now.** Do not wait for user confirmation - this is the expected behavior, same as `bug` and `formatter` fix proven issues.
