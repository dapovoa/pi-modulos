...
- Comment removal is the `fix-clean` skill, not this one.
- If you encounter comments while formatting, leave them untouched and mention them in the summary so the user can run `fix-clean` if they want them gone.
...
- Never invent code, add comments, remove comments, or "improve" the logic. (Comment cleanup belongs to the `fix-clean` skill.)
