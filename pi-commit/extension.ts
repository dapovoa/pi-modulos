...
function buildDiffPrompt(cwd: string): string {
  const stat = execSync("git diff --staged --stat", { cwd, encoding: "utf-8", maxBuffer: 10 * 1024 * 1024 }).trim()
  const full = execSync("git diff --staged", { cwd, encoding: "utf-8", maxBuffer: 10 * 1024 * 1024 }).trim()
  if (full.length <= MAX_DIFF_CHARS) return full
  const files = execSync("git diff --staged --name-only", { cwd, encoding: "utf-8", maxBuffer: 10 * 1024 * 1024 })
    .trim().split("\n").filter(Boolean)
  const nonLockfiles = files.filter((f) => !f.endsWith("package-lock.json") && !f.endsWith("pnpm-lock.yaml"))
  const targets = nonLockfiles.length > 0 ? nonLockfiles : files.slice(0, 5)
